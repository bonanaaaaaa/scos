...
- PlanetScale Postgres and Hyperdrive (#28): region `ap-southeast` (AWS Singapore), a PS-5 single-node cluster (PS-10 if the measured `max_connections` does not fit the connection budget), `main` as the production branch with no development branch, one Hyperdrive configuration with caching disabled, pointed at the direct primary on port 5432 without PlanetScale PgBouncer, and `origin_connection_limit` starting at 5, revisited in #33. The estimated cost is $5 to $16 a month, excluding Cloudflare charges. Details, the connection budget rule and #15's inputs: [Cloudflare deployment design](cloudflare-deployment-design.md).
