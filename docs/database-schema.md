...
`order_allocations` has a unique `(order_id, warehouse_id)` pair: one allocation per warehouse per Order. It stores no warehouse name: the API resolves one through `warehouse_id` (see [Normalization](#normalization-3nf)).
...
- The API returns each allocation's warehouse **name**, and no table stores one against an allocation: `order_allocations.warehouse_id` already identifies the warehouse, and its foreign key is `ON DELETE RESTRICT`, so the related row always exists and `warehouses_name_check` guarantees it is non-blank. Copying the name onto every allocation row would store a fact that depends on `warehouse_id` rather than on the allocation's own key, which is the transitive dependency 3NF exists to remove. The consequence is explicit in the API contract rather than hidden: an allocation's `warehouseName` is the warehouse's **current** name, so renaming a warehouse changes that field in later reads of an Order already returned, and a repeat of a `submissionId` is byte-identical in every other field. The genuinely historical facts — `unit_price`, `discount_rate`, `discount_amount`, `shipping_cost` — are stored precisely because they are not derivable from anything current. A warehouse's name is derivable, from a row the allocation already points at.
...
VerifyOrder in `@scos/core` reads Warehouse Inventory through its `InventoryReader` port. `createPrismaInventoryReader(prisma)` implements the port with one statement per read, a single Prisma `findMany` equivalent to `SELECT id, name, latitude, longitude, stock FROM warehouses ORDER BY id`. Prisma 7.10 with `@prisma/adapter-pg` was observed to send exactly that one `SELECT`, with no `BEGIN` and no second statement. It maps `id` to `warehouseId`, `name` to `warehouseName` and `stock` to `available`, and returns a frozen `InventorySnapshot`; Prisma row types do not leave the adapter. The name is read live with the stock because an estimate is advisory: the current name is the right one to show. An accepted Order resolves its names the same way, through each allocation's foreign key (see [Normalization](#normalization-3nf)).
...
- The warehouse rows are locked with `SELECT id, name, latitude, longitude, stock FROM warehouses ORDER BY id FOR UPDATE`. All rows are locked, not only the ones that will be allocated, so every submission queues in the same order and cannot deadlock with another. `name` is selected under the lock so the estimate's plan names the warehouses it was actually built from; it is not written to the allocation.
...
- On acceptance the adapter checks that the allocations sum to the Order quantity, inserts the Order with its allocations, and deducts each allocation with `UPDATE warehouses SET stock = stock - $q WHERE id = $id AND stock >= $q`, requiring exactly one updated row. Either check failing throws and rolls back. It then rereads the Order and returns it through `restoreOrder`, resolving each allocation's warehouse name through its foreign key, so the first response equals any later repeat unless that warehouse is renamed. Allocations are read in `id` order, which is insertion order.
...
- `packages/core` tests VerifyOrder with an in-memory fake `InventoryReader`, including exact valid, excessive-shipping, and insufficient-stock Order Estimates and reverification after inventory changes.
- `packages/persistence` tests the inventory reader's port contract against real PostgreSQL (`inventory-reader.integration.test.ts`), without core use cases or estimate amounts. The reader's single read is the only database access, and the tests above prove it writes nothing and sees current stock; computing any estimate afterwards happens in memory.
...
Each package tests only itself; there is no core-plus-persistence test level. SubmitOrder's outcomes are unit-tested in `packages/core` with a fake `SubmissionStore`, and the Prisma submission store and its error classification are unit-tested in `packages/persistence` against a stubbed client. No test currently runs a submission against real PostgreSQL: concurrency, rollback, the `submission_key` unique-index backstop, and stock deduction are exercised against the database by the full-stack API tests (#11).
