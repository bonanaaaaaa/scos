...
- Creating the database starts daily billing. Hosted deployment is off until the repository variable `DEPLOY_ENABLED` is `true`. Deploys need no human approval, so setting that variable is the provisioning authorization: the first merge to `main` after it creates the database if it does not exist, and turning it off is the kill switch. Later deploys find it and create nothing. Nothing ever deletes the database automatically; deletion is a manual `pscale database delete`.
...
- The `prod` environment has no required reviewers, by the user's choice: once `DEPLOY_ENABLED` is `true`, every merge to `main` deploys to production without a person approving it. Safety rests on CI passing for the same commit, the main-only branch policy, and `DEPLOY_ENABLED` as the switch.
