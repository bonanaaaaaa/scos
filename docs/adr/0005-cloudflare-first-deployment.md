...
- **Each resource has one owner.** A person runs a `pscale` CLI bootstrap script once to create the PlanetScale database (billed through the Cloudflare account) and its runtime and migration roles. Terraform manages the Cloudflare side (the Hyperdrive configuration and any DNS), with state in a private R2 bucket. Wrangler deploys the Worker.
- **Migrations connect directly to PlanetScale** with the migration role, not through Hyperdrive.

Consequences:

- Creating the database starts daily billing, so it stays a deliberate, authorized, human-run step. No pipeline job creates or deletes the database.
- The pipeline holds no PlanetScale API token. It does hold scoped Cloudflare tokens as GitHub environment secrets. Unless GitHub OIDC federation turns out to be available, that is a downgrade from the AWS design's short-lived credentials, and it is accepted.
