...
# 0.365 kg per unit, $0.01 per kg per km; the sum is rounded half-up to cents,
# once, at the end — never per warehouse
shipping = 0
for warehouse in plan:
    shipping += warehouse.units * 0.365 * 0.01 * warehouse.distance_km
shipping = round(shipping, 2)

total = discounted + shipping
valid = every_unit_allocated and shipping <= 0.15 * discounted
```
