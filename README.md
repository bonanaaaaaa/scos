...
Application and adapter package exports point to compiled files in each package's `dist` directory. All packages use `NodeNext` module resolution, so relative imports keep `.js` extensions. `packages/core` and `apps/api` compile with `tsc`; `packages/persistence` is bundled with [tsdown](https://tsdown.dev) (ESM, declarations generated by the TypeScript 7 native compiler). Its runtime dependencies (`pg`, `@prisma/client`, `@prisma/adapter-pg`, `@scos/core`) stay external. Turbo treats the shared TypeScript configuration as a global dependency so compiler-policy changes invalidate affected cached tasks.
