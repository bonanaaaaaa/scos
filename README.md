...
Interactive API documentation is served at `http://localhost:8080/docs` and the OpenAPI 3.1 specification at `/openapi.json`. The committed copy, [docs/openapi.json](docs/openapi.json), is generated by hono-openapi from the route contracts without a server or database: run `pnpm openapi:export` after changing the API and `pnpm openapi:check` to confirm it is current (`pnpm test` also fails on drift).
